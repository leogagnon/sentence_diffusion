...
def cluster_feats(
    ...
    """
    kmeans = KMeans(n_clusters=num_clusters, n_init=num_redo, max_iter=max_iter, seed=seed+2)
...
class KMeans:
    ...
    def fit_once(self, encodings, init):
        ...
        for i, idx in enumerate(
            torch.randperm(unique_encodings.shape[0], generator=self.generator).tolist()
        ):
            if i == 0:
    ...
    def group_points(self, centers, encodings, capacity=int(1e10)):
        ...
        split_num = math.ceil(encodings.shape[0] / split_len)
        group_index = []
        loss = 0.0
