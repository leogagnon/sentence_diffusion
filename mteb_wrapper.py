...
WANDB_PROJECT = "guillaume-lajoie/sem_emb"
...
def load_encoder_from_checkpoint(
    ...
    from tasks.distill_simcse import DistillSimCSETask
    ...
    if "task" in config and config["task"]["ae"] is not None:
        ...
        task_type = "simcse"
    elif "task" in config and config["task"]["distill_simcse"] is not None:
        task_type = "distill_simcse"
    else:
        raise ValueError(
            f"Could not detect task type from wandb config for run {wandb_id}. "
            f"Expected 'task.ae' or 'task.declutr' or 'task.dino_mixture' or 'task.simcse' or 'task.distill_simcse' in config."
        )
    ...
    elif task_type == "distill_simcse":
        task = DistillSimCSETask.load_from_checkpoint(
            ckpt_path, strict=False, map_location=torch.device(device)
        )
    else:
